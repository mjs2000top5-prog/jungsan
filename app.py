...
def get_gspread_client():
    try:
        ...
        st.error(f"⚠️ 구글 인증 정보를 찾을 수 없습니다. Secrets 설정을 확인해주세요. 오류: {e}")
        return None
...
def process_excel(uploaded_file, columns, input_password):
    ...
    try:
        ...
        if file_extension == 'xlsx':
            ...
            # 지수 표현 방지를 위해 dtype=str 사용
            df = pd.read_excel(decrypted_workbook, usecols=columns, dtype=str, index_col=None).fillna('')
...
def run_upload_ui(title, columns, sheet_name):
    ...
    if uploaded_file:
        ...
        if f'df_{sheet_name}' in st.session_state:
            ...
            if st.button(f"🚀 {sheet_name} 시트로 최종 업로드 (첫 행 포함)"):
...
if menu == "정산 데이터 생성":
            ...
            with st.spinner('데이터 분석 및 정산 중...'):
                ...
                if client:
                    ...
                    if len(gaib_raw) < 2:
                        ...
                        # 기본 필터링
                        df_gaib = df_gaib[
                            (df_gaib.iloc[:, 10] != 'TEST') & (df_gaib.iloc[:, 7] != '휴폐업') & (df_gaib.iloc[:, 2] != '위멤버스 베이직')
                        ]
                        df_gaib = df_gaib[df_gaib.iloc[:, 3].apply(lambda x: str(x).replace("-", "")[:6] != prev_month_str)]
                        ...
                        def calculate_final(row):
                            ...
                            if gaib_no in special_map:
                                try:
                                    price = int(str(special_map[gaib_no]).replace(",", ""))
                                    return pd.Series([price, int(price * 0.1)])
                                except: pass
                            ...
                            try:
                                ...
                                return pd.Series([final, int(final * 0.1)])
    ...
    if 'result_df' in st.session_state:
        ...
        st.download_button("📥 정산 결과 엑셀 다운로드", output.getvalue(), f"정산결과_{target_month}.xlsx", "application/vnd.ms-excel")
    ...
    target = st.radio("대상", ["위멤버스 가입자", "위멤버스 사용자"])
    if st.button("🔥 삭제"):
        client = get_gspread_client()
        if client:
            client.open_by_key(SPREADSHEET_ID).worksheet(target).clear()
            st.success("완료")